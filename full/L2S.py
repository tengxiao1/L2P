...
parser.add_argument('--dataset', type=str,
                    default='cornell')  # squirrel film chameleon     cornell   texas   wisconsin# cora PubMed CiteSeer citeseer
...
def allTrain(data, adj):
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best:
            ...
            # #                    args.dropout) + '.csv'
            # checkpt_file = '%sresultPiSoftmaxTrue%s__' % (
...
checkpt_file = 'fullresult/%s%sfullresultpiTenSoft_' % (
args.laynumber, args.taskType) + args.dataset + '_ConcatbestTest_%s' % np.mean(
    acc_list) + '_H0alpha%s_entropy%s_alpha%s_weight_decay%s_hidden%s_dropout%s' % (
